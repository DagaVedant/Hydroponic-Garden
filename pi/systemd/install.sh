#!/usr/bin/env bash
# Install the three services and start them. Run from anywhere, as root:
#
...
if [ ! -x "$PI_DIR/.venv/bin/python" ]; then
    ...
    echo "  $PI_DIR/.venv/bin/pip install -r $PI_DIR/requirements.txt" >&2
    exit 1
fi

for unit in control store web; do
